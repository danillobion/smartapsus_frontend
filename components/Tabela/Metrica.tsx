...
import Cabecalho from '../Layout/Interno/Cabecalho';

const Tabela = ({ dados, carregarRegistros, IrParaTelaAdicionarRegistro, IrParaTelaEditarRegistro, deletarRegistro }) => {
  ...
  return (
    <div>
      ...
      {/* x cabecalho x */}
      {/* tabela */}
      <div className='border border-gray-300 rounded-md pb-1 shadow-sm'>
            ...
            <tr>
              <th scope="col" className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                ...
                <h6>
                  ...
                  <button onClick={(e) => paramsColuna('sort', body_params.sort != null && body_params.sort.split(',')[1] == 'asc' ? 'nome,desc' : 'nome,asc' )}>
                    {body_params.sort != null && body_params.sort.split(',')[1] == 'asc' ? '▲' : '▼'}
                  </button>
                </h6>                  
                <input
                      type="text"
              ...
              <th scope="col" className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                <div className='w-full'>
                  <h6>
                    Detalhes 
                    <button onClick={(e) => paramsColuna('sort', body_params.sort != null && body_params.sort.split(',')[1] == 'asc' ? 'detalhes,desc' : 'detalhes,asc' )}>
                      {body_params.sort != null && body_params.sort.split(',')[1] == 'asc' ? '▲' : '▼'}
                    </button>
                  </h6>  
                  <input
                      ...
                      id="detalhes"
                      className="mt-1 p-2 w-full border rounded"
                      onChange={(e) => paramsColuna('detalhes', e.target.value)}
                  />
                </div>
              </th>
              <th scope="col" className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
            ...
            {dados.content.length > 0 ? (
              dados.content.map((item) => (
                <tr key={item.id} className="hover:bg-gray-100">
                  ...
                  <td className="px-3 py-2 whitespace-nowrap">{item.detalhes}</td>
