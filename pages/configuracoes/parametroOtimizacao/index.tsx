...
const estrutura = {
  ...
  cabecalho:{ //cabecalho da pagina
    titulo:"Parâmetro de Otimização",
    migalha:[
      {nome:'Mapa', link:'/'},
      {nome:'Configurações', link:null},
      {nome:'Parâmetro de Otimização', link:null},
    ]
  },
  ...
  tabela:{
    ...
    colunas:[ //colunas da tabela
      ...
      {nome:"tipo de dado",chave:"tipoDado",tipo:"texto",selectOptions:null,sort:true,pesquisar:true},
...
const Lista = () => {
  ...
  const deletarRegistro = async (item: Item) => {
    const confirmacao = await Swal.fire({
      title: "Você deseja deletar o parâmetro de otimização?",
    ...
    if (confirmacao.isConfirmed) {
      try {
        ...
        if (response.data.errors) {
          ...
          pesquisarRegistro();
          Swal.fire({
            title: "Parâmetro de otimização deletado com sucesso",
            icon: "success"
          });
        }
      } catch (error) {
